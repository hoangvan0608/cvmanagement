...
public class ExportServiceImpl implements ExportService {
    ...
    @Override
    public ResponseEntity<InputStreamResource> exportExcel(GetListTicketRequest getListTicketRequest) throws IOException {
                ...
                if (getListTicketRequest.getDepId() != 0) {
                    ...
                    if (depOpt.isPresent()) {
                        for (TicketEntity ticket : lst) {
                            ...
                            if (lstDep.stream().allMatch(e -> e.getDepEntity().getDepId() != getListTicketRequest.getDepId())) {
                                lstObj.add(ticket);
                            }
                            List<TicketUserRelationEntity> lstUser = ticket.getPIC();
                            if(lstUser.stream().allMatch(e->e.getUserEntity().getDepEntity().getDepId() != getListTicketRequest.getDepId())){
                                lstObj.add(ticket);
                            }
                        }
                        if (lstObj.size() > 0) {
...
//                        lst.removeIf(t -> (t.getListDeps().stream().filter(e->e.getDepEntity().getDepId() != getListTicketRequest.getDepId()).);
        ...
        File file = new File("/tmp/fileCV/" + "DemoFile.xlsx");
        ...
        try {
            fileCvStream = new InputStreamResource(new FileInputStream(file));
        }
        catch (Exception c)
        {
            c.printStackTrace();
        ...
        headers.add("Content-Disposition", String.format("attachment; filename =\"%s\"",new Date().getTime() + ".xlsx"));
